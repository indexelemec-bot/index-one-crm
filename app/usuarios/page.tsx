...
import { AlertTriangle, CheckCircle2, KeyRound, Mail, Pencil, Shield, Trash2, UserPlus } from "lucide-react";
...
export default function UsuariosPage() {
  ...
  async function changePassword(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!passwordUserId || !canManage) return;
    const form = new FormData(event.currentTarget);
    const password = String(form.get("password") ?? "");
    const confirmation = String(form.get("confirmation") ?? "");
    setError(""); setMessage("");
    if (password.length < 10 || password.length > 128) { setError("La contraseña debe tener entre 10 y 128 caracteres."); return; }
    ...
    try {
      const response = await fetch("/api/admin/users/password", {
        ...
        body: JSON.stringify({ userId: passwordUserId, password, confirmation })
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) { setError(result.error ?? "No fue posible cambiar la contraseña."); return; }
  ...
  return <>
    <PageHeader eyebrow="Seguridad y acceso" title="Usuarios, roles y permisos" description="Invita al equipo, cambia roles y contraseñas, bloquea accesos y elimina usuarios con reasignación obligatoria de cartera."><button className="button button-primary" disabled={!canManage} onClick={() => { setOpen(true); setError(""); }}><UserPlus size={18}/> Invitar usuario</button></PageHeader>
    {message && <div className="sync-banner"><CheckCircle2 size={15}/> {message}</div>}
    {error && !open && !editingId && !editingAccessId && !deletingId && !passwordUserId && <div className="sync-banner sync-error">{error}</div>}
    ...
    {passwordUserId && passwordUser && canManage && <Modal title={`Cambiar contraseña de ${passwordUser.fullName}`} description="La nueva contraseña se aplicará inmediatamente y el cambio quedará registrado en auditoría." onClose={() => { setPasswordUserId(null); setError(""); }}><form onSubmit={changePassword}>{error && <div className="sync-banner sync-error">{error}</div>}<div className="formal-note"><KeyRound size={18}/><span>La contraseña no se guardará en el CRM ni aparecerá en el registro de auditoría.</span></div><div className="form-grid"><label className="field field-wide"><span>Nueva contraseña</span><input name="password" type="password" autoComplete="new-password" minLength={10} maxLength={128} autoFocus required/></label><label className="field field-wide"><span>Confirmar contraseña</span><input name="confirmation" type="password" autoComplete="new-password" minLength={10} maxLength={128} required/></label></div><div className="form-actions"><button type="button" className="button" onClick={() => { setPasswordUserId(null); setError(""); }}>Cancelar</button><button className="button button-primary" disabled={sending}>{sending ? "Actualizando…" : "Cambiar contraseña"}</button></div></form></Modal>}
