"use client";
import { CheckCircle2, Mail, Pencil, Shield, UserPlus } from "lucide-react";
...
export default function UsuariosPage() {
  const { users, currentUser, toggleUser, inviteUser, updateUserProfile } = useCrm();
  const [open, setOpen] = useState(false); const [sending, setSending] = useState(false);
  const [error, setError] = useState(""); const [message, setMessage] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null); const [profileName, setProfileName] = useState("");
