...
export default function UpdatePasswordPage() {
  ...
  return <main className="login-page"><section className="login-brand"><div className="login-brand-inner"><div className="login-logo"><span>⌂</span> INDEX <b>ONE</b></div><p className="login-kicker">ACCESO SEGURO</p><h1>Activa tu acceso al <em>CRM comercial</em>.</h1><p>Define una contraseña personal para proteger prospectos, decisores, oportunidades y propuestas.</p><div className="login-proof"><ShieldCheck/><div><b>Sesión protegida por Supabase</b><small>Tu contraseña no se almacena en INDEX ONE CRM.</small></div></div></div></section><section className="login-form-wrap"><form className="login-form" onSubmit={submit}><div className="login-mobile-logo">INDEX <b>ONE</b></div><span className="eyebrow">Activación de cuenta</span><h2>Establece tu contraseña</h2><p>Usa al menos 10 caracteres y evita reutilizar contraseñas personales.</p><label className="login-field"><span>Nueva contraseña</span><div><LockKeyhole size={18}/><input name="password" type={visible?"text":"password"} autoComplete="new-password" required disabled={!ready}/><button type="button" onClick={()=>setVisible(!visible)} aria-label="Mostrar contraseña">{visible?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></label><label className="login-field"><span>Confirmar contraseña</span><div><LockKeyhole size={18}/><input name="confirmation" type={visible?"text":"password"} autoComplete="new-password" required disabled={!ready}/></div></label>{error&&<div className="field-error">{error}</div>}<button className="button button-primary login-button" disabled={!ready||loading}>{loading?"Guardando…":<>Guardar contraseña <ArrowRight size={18}/></>}</button><small className="demo-note">{ready?"Enlace validado. Ya puedes definir tu contraseña.":"Validando enlace seguro…"}</small></form></section></main>;
}
