...
function proposalHistory(proposals: Proposal[], opportunityId: string) {
  return proposals
    .filter((proposal) => proposal.opportunityId === opportunityId)
...
function variationLabel(proposal: Proposal, history: Proposal[]) {
  ...
  if (!difference)
    return proposal.version === 1 ? "Monto inicial" : "Sin variación";
...
export default function PropuestasPage() {
  ...
  const [deliveryChannel, setDeliveryChannel] = useState<"email" | "whatsapp">(
    "email",
  );
  ...
  const history = useMemo(
    () => proposalHistory(proposals, opportunityId),
    [proposals, opportunityId],
  );
  const previous = history.at(-1);
  const initial = history[0];
  const priceChanged = Boolean(previous && previous.monthlyFee !== fee);
  const suggestions = useMemo(
    () => (account ? selectComparableReferences(account, references, references.length) : []),
    [account, references],
  );
  const orderedProposals = useMemo(
    () =>
      [...proposals].sort((a, b) => b.generatedAt.localeCompare(a.generatedAt)),
    [proposals],
  );
  const deliveryOpportunity = opportunities.find(
    (item) => item.id === deliveryProposal?.opportunityId,
  );
  const deliveryContacts = stakeholders.filter(
    (item) => item.accountId === deliveryOpportunity?.accountId,
  );
  const selectedDeliveryContact = deliveryContacts.find(
    (item) => item.id === deliveryStakeholder,
  );
  ...
  function selectOpportunity(id: string) {
    ...
    setFee(
      selectedHistory.at(-1)?.monthlyFee ||
        selectedOpportunity?.monthlyFee ||
        36000,
    );
    setChangeReason("");
  }
  ...
  useEffect(() => {
    if (account)
      setSelected(
        selectComparableReferences(account, references, 3).map(
  ...
  function start() {
    const next = opportunities.find(
      (item) => !["cliente_activo", "perdida"].includes(item.stage),
    );
  ...
  async function generate() {
    if (
      ...
      !opportunity ||
      selected.length !== 3 ||
      (priceChanged && !changeReason.trim())
    )
      return;
    setLoading(true);
    try {
      const chosen = selected.map(
        (id) => references.find((reference) => reference.id === id)!,
      );
      ...
      if (!response.ok)
        throw new Error("No fue posible generar el archivo de la propuesta.");
      ...
      const savedProposal: Proposal = {
        ...
        monthlyFee: fee,
        referenceIds: selected,
        referencesSnapshot: chosen,
        projectType: account.projectType,
        residentialSubtype: account.residentialSubtype,
        customUnitType: account.customUnitType,
        status: "generada",
      ...
      if (supabase) {
        const { error: persistenceError } = await supabase
          ...
          .insert({
            ...
            references_snapshot: savedProposal.referencesSnapshot,
            project_type: savedProposal.projectType,
            residential_subtype: savedProposal.residentialSubtype ?? null,
            custom_unit_type: savedProposal.customUnitType ?? null,
        ...
        if (persistenceError)
          throw new Error(
            `La propuesta se generó, pero no pudo guardarse en el CRM: ${persistenceError.message}`,
          );
        await refreshData();
      } else {
      ...
      anchor.href = url;
      anchor.download = `propuesta-index-${account.name
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
      ...
      setToast(
        `Versión ${savedProposal.version} generada, guardada y lista para enviar en ${format === "pdf" ? "PDF" : "Word"}`,
      );
      setOpen(false);
      ...
      setToast(
        error instanceof Error
          ? error.message
          : "No fue posible generar y guardar la propuesta.",
      );
  ...
  function applyDeliveryTemplate(
  ...
  function changeDeliveryChannel(channel: "email" | "whatsapp") {
    ...
    const available = deliveryContacts.find((item) =>
    ...
    applyDeliveryTemplate(
      channel,
      deliveryProposal,
      available ?? selectedDeliveryContact,
    );
  }
  ...
  async function deliverProposal(event: React.FormEvent) {
    ...
    try {
      ...
      const payload =
        deliveryChannel === "email"
          ? {
              opportunityId: deliveryProposal.opportunityId,
              proposalId: deliveryProposal.id,
              stakeholderId: selectedDeliveryContact.id,
              templateKey: "propuesta",
      ...
      if (!response.ok)
        throw new Error(result.error ?? "No fue posible preparar el envío.");
      setDeliveries((items) => [
        mapCommunication({
          ...result.communication,
          proposal_id: deliveryProposal.id,
        }),
        ...items.filter((item) => item.id !== result.communication.id),
      ]);
      ...
      if (deliveryChannel === "whatsapp") {
        if (result.mode === "official") {
          ...
          setToast(
            "Propuesta enviada desde el WhatsApp Business oficial y registrada.",
          );
        } else {
          if (popup) popup.location.href = result.whatsappUrl;
          else window.open(result.whatsappUrl, "_blank", "noopener,noreferrer");
          setToast(
            "Propuesta preparada y abierta en WhatsApp. El intento quedó registrado.",
          );
        }
      } else
        setToast(
          "Propuesta enviada por correo con el archivo adjunto y registrada en el historial.",
        );
      setDeliveryProposal(undefined);
  ...
  return (
    <>
      <PageHeader
        eyebrow="Plantilla corporativa bloqueada"
        title="Propuestas comerciales"
        description="Genera Word o PDF y conserva cada versión para seguir la negociación y sus variaciones de precio."
      >
        <button className="button button-primary" onClick={start}>
          <Plus size={18} /> Generar propuesta
        </button>
      </PageHeader>
        ...
        <table className="table proposal-history-table">
          <thead>
            <tr>
              ...
              <th>Fecha</th>
              <th>Honorarios</th>
              <th>Variación vs. inicial</th>
              <th>Motivo</th>
              <th>Archivo</th>
              <th>Envíos y reenvíos</th>
              <th>Acción</th>
            </tr>
          </thead>
          <tbody>
            {orderedProposals.map((proposal) => {
              const versions = proposalHistory(
                proposals,
                proposal.opportunityId,
              );
              const variation =
                proposal.monthlyFee -
                (versions[0]?.monthlyFee ?? proposal.monthlyFee);
              ...
              return (
                <tr key={proposal.id}>
                  <td>
                    ...
                    <small>
                      <History size={11} /> {versions.length}{" "}
                  ...
                  <td>
                    <span
                      className={`price-variation ${variation < 0 ? "discount" : variation > 0 ? "increase" : "initial"}`}
                    >
                      {variationLabel(proposal, versions)}
                    </span>
                  </td>
                  <td>
                    {proposal.changeReason || (
                      <span className="muted-copy">Primera propuesta</span>
                    )}
                  </td>
                  <td>
                    <span className="file-format">
                      <FileText size={13} />
                      {proposal.fileFormat === "pdf" ? "PDF" : "Word"}
                    </span>
                    <small>{proposal.status}</small>
                  </td>
                  <td>
                    {attempts.length === 0 ? (
                      <span className="muted-copy">Sin enviar</span>
                    ) : (
                      <div className="proposal-deliveries">
                        {attempts.map((attempt, index) => {
                          const attemptNumber = attempts.length - index;
                          return (
                            <span
                              className={`proposal-delivery ${attempt.status === "failed" ? "failed" : ""}`}
                              key={attempt.id}
                            >
                  ...
                  <td>
                    <div className="proposal-actions">
                      <a
                        ...
                        href={`/api/proposals/download?id=${proposal.id}`}
                      >
                        <Download size={14} />
                        Descargar
                      </a>
                      <button
                        className="button compact"
                        onClick={() => openDelivery(proposal)}
                      >
                        <Send size={14} />
                        {attempts.length ? "Reenviar" : "Enviar"}
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {open && (
        <Modal
          title="Generar propuesta oficial"
          description="Elige el formato. Cada generación crea una nueva versión inalterable en el historial."
        ...
        >
            ...
            <section className="proposal-form">
                ...
                <label className="field field-wide">
                  ...
                  >
                    ...
                    {opportunities
                      .filter(
                        (item) =>
                          !["cliente_activo", "perdida"].includes(item.stage),
                      )
                      .map((item) => (
              ...
              <div className="format-section">
                ...
                <div className="format-picker">
                  ...
                  <label
                    className={`format-option ${format === "docx" ? "selected" : ""}`}
                  >
                    <input
                      type="radio"
                      name="format"
                    ...
                    <span>
                      <b>Word</b>
                      <small>Documento editable</small>
                    </span>
                  </label>
                </div>
              </div>
              ...
              {previous && (
                <div className="negotiation-summary">
                  <div>
                    ...
                    <b>{formatCurrency(initial.monthlyFee)}</b>
                  </div>
                  <div>
                    <small>Última versión</small>
                    <b>{formatCurrency(previous.monthlyFee)}</b>
                  </div>
                  <div>
                    <small>Nueva versión</small>
                    <b
                      className={
                        fee < initial.monthlyFee ? "discount-text" : ""
                      }
                    >
                      {formatCurrency(fee)}
                    </b>
              ...
              {priceChanged && (
                <label className="field" style={{ marginTop: 14 }}>
                  <span>Motivo de la variación</span>
                  <textarea
                    value={changeReason}
                    onChange={(event) => setChangeReason(event.target.value)}
                    required
                    placeholder="Ej. Rebaja solicitada por la junta durante la negociación"
                  />
                  <small className="field-error">
                    Este motivo quedará guardado en el historial comercial.
                  </small>
                </label>
              )}
              ...
              {account && (
                <>
                  ...
                  >
                    <Sparkles size={15} color="#f47721" />
                    <b style={{ fontSize: 12 }}>
                      Referencias comparables sugeridas
                    </b>
                  </div>
                  <div className="reference-list">
                    {suggestions.map((reference) => (
                      <label className="reference-option" key={reference.id}>
                        <input
                          type="checkbox"
                        ...
                        />
                        <span>
                          <b>{reference.clientName}</b>
                          <small>
                            {reference.units}{" "}
                            {reference.projectType === "comercial"
                              ? "locales"
                              : reference.residentialSubtype === "casa"
                                ? "casas"
                                : reference.residentialSubtype === "otros"
                                  ? reference.customUnitType
                                  : "apartamentos"}{" "}
                            · {reference.location}
                          </small>
                        </span>
                        <span className="match">{reference.score}%</span>
                      </label>
                    ))}
                  </div>
                  <small
                    style={{
                      color: selected.length === 3 ? "#1b8a5a" : "#c9364f",
                    }}
                  >
                    {selected.length}/3 referencias seleccionadas
                  </small>
                </>
              )}
              ...
              <div className="paper">
                ...
                <h2>
                  ...
                  {account?.projectType === "comercial"
                    ? "DE PROYECTO COMERCIAL"
                    : `RESIDENCIAL · ${account?.residentialSubtype === "casa" ? "CASAS" : account?.residentialSubtype === "otros" ? account.customUnitType?.toUpperCase() : "APARTAMENTOS"}`}
                </h2>
                <div className="orange">PLAN DE ADMINISTRACIÓN INTEGRAL</div>
                ...
                <div className="paper-price">{formatCurrency(fee)} / mes</div>
              </div>
            </section>
          </div>
          <div
          ...
          >
            <FileCheck2 size={15} /> El archivo maestro permanece sin cambios;
            cada propuesta se genera como una copia versionada.
          </div>
        </Modal>
      )}
      {deliveryProposal && (
        <Modal
          title={`${deliveries.some((item) => item.proposalId === deliveryProposal.id) ? "Reenviar" : "Enviar"} propuesta v${deliveryProposal.version}`}
          description={`El envío quedará vinculado a esta versión de ${deliveryProposal.clientName}.`}
          onClose={() => setDeliveryProposal(undefined)}
          wide
        >
          <form onSubmit={deliverProposal}>
            {deliveryError && (
              <div className="sync-banner sync-error">{deliveryError}</div>
            )}
            <div className="delivery-channel-picker">
              <button
                type="button"
                className={deliveryChannel === "email" ? "active" : ""}
                onClick={() => changeDeliveryChannel("email")}
              >
                <Mail size={20} />
                <span>
                  <b>Correo</b>
                  <small>Archivo adjunto y estado de entrega</small>
                </span>
              </button>
              <button
                type="button"
                className={deliveryChannel === "whatsapp" ? "active" : ""}
                onClick={() => changeDeliveryChannel("whatsapp")}
              >
                <MessageCircle size={20} />
                <span>
                  <b>WhatsApp</b>
                  <small>Mensaje y enlace privado por 7 días</small>
                </span>
              </button>
            </div>
            <div className="form-grid">
              <label className="field field-wide">
                <span>Contacto</span>
                <select
                  value={deliveryStakeholder}
                  onChange={(event) =>
                    changeDeliveryStakeholder(event.target.value)
                  }
                  required
                >
              ...
              <div className="formal-note field-wide">
                <FileCheck2 size={16} /> Se enviará exactamente la versión{" "}
                {deliveryProposal.version} en{" "}
                {deliveryProposal.fileFormat === "pdf" ? "PDF" : "Word"}. Cada
                nuevo intento aparecerá por separado en el historial.
              </div>
              {deliveryChannel === "whatsapp" && (
                <div className="whatsapp-disclosure field-wide">
                  <MessageCircle size={16} /> El CRM abrirá WhatsApp con el
                  mensaje listo. Se registrará la apertura del canal; la
                  confirmación final depende de pulsar “Enviar” en WhatsApp.
                </div>
              )}
            </div>
            <div className="form-actions">
              <button
                type="button"
                className="button"
                onClick={() => setDeliveryProposal(undefined)}
              >
                Cancelar
              </button>
              <button
                ...
                disabled={
                  ...
                  (deliveryChannel === "email"
                    ? !selectedDeliveryContact.email
                    : !selectedDeliveryContact.phone)
                }
              >
                <Send size={16} />
                {delivering
