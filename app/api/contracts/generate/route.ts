...
const schema = z.object({ opportunityId: z.string().uuid(), clientLegalName: z.string().trim().min(3), clientRnc: z.string().trim().min(5), clientAddress: z.string().trim().min(5), city: z.string().trim().min(2), sector: z.string().trim().min(2), representativeName: z.string().trim().min(3), representativeId: z.string().trim().min(5), representativeGenderEnding: z.enum(["o", "a"]), assemblyDate: z.string().date(), monthlyFee: z.number().positive(), monthlyFeeWords: z.string().trim().min(5), signatureDate: z.string().date(), changeReason: z.string().trim().min(3), negotiatedTerms: z.string().trim().max(12000).optional() });
const contractMasterSha256 = "1297909bcb760dd41f35dedc6468d72c1d650fd94f08a24600ee0afd877a6f74";

export async function POST(request: Request) {
  const parsed = schema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return NextResponse.json({ error: "Revisa los datos legales, fechas, honorarios y motivo de versión." }, { status: 400 });
  ...
  const version = Number(contract.current_version) + 1; let bytes: Buffer; try { bytes = await buildContract(parsed.data); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "No fue posible generar el contrato." }, { status: 500 }); }
  ...
  const inserted = await supabase.from("contract_versions").insert({ contract_id: contract.id, version, monthly_fee: parsed.data.monthlyFee, change_reason: parsed.data.changeReason, negotiated_terms: parsed.data.negotiatedTerms || null, file_path: path, master_sha256: contractMasterSha256, generated_by: authData.user.id }).select("*").single(); if (inserted.error) return NextResponse.json({ error: inserted.error.message }, { status: 500 });
  ...
  const signed = await admin.storage.from("contract-files").createSignedUrl(path, 60 * 15); return NextResponse.json({ contractId: contract.id, version, signedUrl: signed.data?.signedUrl });
}
