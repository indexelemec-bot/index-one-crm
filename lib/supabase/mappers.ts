import type {
  ...
  CommercialReference,
  Communication,
  CommunicationAssignmentHistory,
  CommunicationThread,
  MarketingLead,
  Opportunity,
  Proposal,
...
export const mapAccount = (row: Record<string, unknown>): Account => ({
  ...
  projectType: row.project_type === "comercial" ? "comercial" : "residencial",
  residentialSubtype:
    (row.residential_subtype as Account["residentialSubtype"]) || undefined,
  customUnitType: row.custom_unit_type
    ? String(row.custom_unit_type)
    : undefined,
...
export const mapStakeholder = (row: Record<string, unknown>): Stakeholder => ({
  ...
  fullName: String(row.full_name),
  role: row.role as Stakeholder["role"],
  phone: String(row.phone ?? ""),
  email: String(row.email ?? ""),
  influence: Number(row.influence),
  position: row.position as Stakeholder["position"],
...
export const mapOpportunity = (row: Record<string, unknown>): Opportunity => ({
  ...
  nextFollowupAt: row.next_followup_at
    ...
    : undefined,
  followupIntervalMonths: row.followup_interval_months
    ? Number(row.followup_interval_months)
    : undefined,
});
...
export const mapProposal = (row: Record<string, unknown>): Proposal => ({
  ...
  referencesSnapshot: Array.isArray(row.references_snapshot)
    ? (row.references_snapshot as unknown as Proposal["referencesSnapshot"])
    : undefined,
  projectType: (row.project_type as Proposal["projectType"]) || undefined,
  residentialSubtype:
    (row.residential_subtype as Proposal["residentialSubtype"]) || undefined,
  customUnitType: row.custom_unit_type
    ? String(row.custom_unit_type)
    : undefined,
  fileFormat: row.file_format === "pdf" ? "pdf" : "docx",
...
export const mapReference = (
  ...
  units: Number(row.units),
  accountType: row.account_type as CommercialReference["accountType"],
  projectType: row.project_type === "comercial" ? "comercial" : "residencial",
  residentialSubtype:
    (row.residential_subtype as CommercialReference["residentialSubtype"]) ||
    undefined,
  customUnitType: row.custom_unit_type
    ? String(row.custom_unit_type)
    : undefined,
  ...
  incorporatedAt: String(
    row.incorporated_at ?? new Date().toISOString().slice(0, 10),
  ),
  notes: row.notes ? String(row.notes) : undefined,
  contactShareAuthorized: Boolean(row.contact_share_authorized),
});
...
export const mapSalesReport = (row: Record<string, unknown>): SalesReport => ({
  ...
  initialFee: Number(row.initial_fee),
  finalFee: Number(row.final_fee),
  annualValue: Number(row.annual_value),
  commissionRate: Number(row.commission_rate),
  commissionBase: Number(row.commission_base),
  commissionAmount: Number(row.commission_amount),
  commissionStatus: row.commission_status as SalesReport["commissionStatus"],
  firstPaymentReceivedAt: row.first_payment_received_at
    ? String(row.first_payment_received_at)
    : undefined,
  commissionPaidAt: row.commission_paid_at
    ? String(row.commission_paid_at)
    : undefined,
...
export const mapAssignmentHistory = (
  row: Record<string, unknown>,
): AssignmentHistory => ({
  id: String(row.id),
  opportunityId: String(row.opportunity_id),
  previousOwnerId: row.previous_owner_id
    ? String(row.previous_owner_id)
    : undefined,
  newOwnerId: String(row.new_owner_id),
...
export const mapSpeechUsage = (row: Record<string, unknown>): SpeechUsage => ({
  id: String(row.id),
  speechId: String(row.speech_id),
  opportunityId: String(row.opportunity_id),
  stakeholderId: row.stakeholder_id ? String(row.stakeholder_id) : undefined,
  stage: row.stage as SpeechUsage["stage"],
  userId: String(row.user_id),
  channel: row.channel as SpeechUsage["channel"],
  outcome: row.outcome as SpeechUsage["outcome"],
  notes: row.notes ? String(row.notes) : undefined,
  nextAction: String(row.next_action),
  nextActionAt: String(row.next_action_at),
...
export const mapCommunication = (
  ...
  providerMediaId: row.provider_media_id
    ? String(row.provider_media_id)
    : undefined,
  attachmentFormat:
    row.attachment_format === "pdf" || row.attachment_format === "docx"
  ...
  agentNameSnapshot: row.agent_name_snapshot
    ? String(row.agent_name_snapshot)
    : undefined,
  ...
  mediaMime: row.media_mime_type ? String(row.media_mime_type) : undefined,
  transcriptionText: row.transcription_text
    ? String(row.transcription_text)
    : undefined,
  transcriptionStatus:
    row.transcription_status as Communication["transcriptionStatus"],
  transcriptionError: row.transcription_error
    ? String(row.transcription_error)
    : undefined,
  transcriptionProvider: row.transcription_provider
    ? String(row.transcription_provider)
    : undefined,
  transcriptionLanguage: row.transcription_language
    ? String(row.transcription_language)
    : undefined,
  transcriptionCompletedAt: row.transcription_completed_at
    ? String(row.transcription_completed_at)
    : undefined,
  replyToProviderMessageId: row.reply_to_provider_message_id
    ? String(row.reply_to_provider_message_id)
    : undefined,
...
export const mapClientDocument = (
  ...
  opportunityId: String(row.opportunity_id),
  stakeholderId: row.stakeholder_id ? String(row.stakeholder_id) : undefined,
  templateKey: row.template_key as ClientDocument["templateKey"],
  title: String(row.title),
  fileName: String(row.file_name),
  dataSnapshot: (row.data_snapshot as Record<string, unknown>) ?? {},
...
export const mapCommunicationThread = (
  row: Record<string, unknown>,
): CommunicationThread => ({
  id: String(row.id),
  opportunityId: String(row.opportunity_id),
...
export const mapCommunicationAssignmentHistory = (
  row: Record<string, unknown>,
): CommunicationAssignmentHistory => ({
  id: String(row.id),
  threadId: String(row.thread_id),
  previousAgentId: row.previous_agent_id
    ? String(row.previous_agent_id)
    : undefined,
...
export const mapScheduledCommunication = (
  row: Record<string, unknown>,
): ScheduledCommunication => ({
  id: String(row.id),
  threadId: row.thread_id ? String(row.thread_id) : undefined,
  opportunityId: String(row.opportunity_id),
  stakeholderId: String(row.stakeholder_id),
  channel: row.channel as ScheduledCommunication["channel"],
  bodyText: String(row.body_text ?? ""),
  templateKey: row.template_key ? String(row.template_key) : undefined,
  ...
  recurrenceMonths: row.recurrence_months
    ? Number(row.recurrence_months)
    : undefined,
  status: row.status as ScheduledCommunication["status"],
  createdBy: String(row.created_by),
  sentCommunicationId: row.sent_communication_id
    ? String(row.sent_communication_id)
    : undefined,
...
export const mapMarketingLead = (
  row: Record<string, unknown>,
): MarketingLead => ({
  id: String(row.id),
  provider: String(row.provider ?? "meta"),
  sourceChannel: String(row.source_channel ?? "other"),
  ...
  condominiumName: row.condominium_name
    ? String(row.condominium_name)
    : undefined,
  sector: row.sector ? String(row.sector) : undefined,
  units: row.units ? Number(row.units) : undefined,
  primaryProblem: row.primary_problem ? String(row.primary_problem) : undefined,
  stakeholderRole: row.stakeholder_role
    ? String(row.stakeholder_role)
    : undefined,
  boardMember:
    row.board_member === null || row.board_member === undefined
      ? undefined
      : Boolean(row.board_member),
  wantsAssessment:
    row.wants_assessment === null || row.wants_assessment === undefined
      ? undefined
      : Boolean(row.wants_assessment),
...
export const mapArmAgent = (row: Record<string, unknown>): ArmAgent => ({
  ...
  kind: row.kind as ArmAgent["kind"],
  roleKey: String(row.role_key),
  description: String(row.description),
  status: row.status as ArmAgent["status"],
  autonomyLevel: row.autonomy_level as ArmAgent["autonomyLevel"],
  riskLevel: row.risk_level as ArmAgent["riskLevel"],
...
export const mapArmAssignment = (
  ...
  opportunityId: String(row.opportunity_id),
  relationshipRole:
    row.relationship_role as ArmAgentAssignment["relationshipRole"],
  status: row.status as ArmAgentAssignment["status"],
  notes: row.notes ? String(row.notes) : undefined,
...
export const mapArmInteraction = (
  row: Record<string, unknown>,
): ArmInteraction => ({
  id: String(row.id),
  agentId: String(row.agent_id),
  ...
  decisionStatus: row.decision_status as ArmInteraction["decisionStatus"],
  confidenceScore:
