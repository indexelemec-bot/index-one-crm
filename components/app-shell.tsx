...
import { BadgeDollarSign, BookOpenCheck, Building2, CheckSquare2, ChevronDown, ChevronLeft, ChevronRight, FileSignature, Gauge, KanbanSquare, Menu, MessageSquareText, Plus, ScrollText, Settings, UserRoundCog, X } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const [mobileOpen, setMobileOpen] = useState(false); const [collapsed, setCollapsed] = useState(false); const { currentUser, users, setCurrentUser, resetDemo, loading, syncError } = useCrm();

  useEffect(() => { setCollapsed(localStorage.getItem("sidebar-collapsed") === "1"); }, []);
  const toggleCollapsed = () => setCollapsed((c) => { localStorage.setItem("sidebar-collapsed", c ? "0" : "1"); return !c; });

  if (pathname === "/login") return <>{children}</>;
  return <div className={`shell ${collapsed ? "shell-collapsed" : ""}`}>
    <aside className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
      <div className="brand"><span className="brand-mark">⌂</span><span>INDEX <b>ONE</b><small>CRM CONDOMINIAL</small></span><button className="icon-button mobile-only" onClick={() => setMobileOpen(false)} aria-label="Cerrar menú"><X size={20}/></button></div>
      <nav className="nav" aria-label="Navegación principal">{nav.map(({ href, label, icon: Icon }) => <Link href={href} onClick={() => setMobileOpen(false)} className={pathname.startsWith(href) ? "active" : ""} key={href}><Icon size={19}/><span>{label}</span></Link>)}</nav>
