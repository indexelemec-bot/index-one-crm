...
export function AppShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const [open, setOpen] = useState(false); const { currentUser, users, setCurrentUser, resetDemo } = useCrm();
  ...
  return <div className="shell">
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <div className="sidebar-footer"><button className="nav-plain" onClick={resetDemo}><Settings size={18}/> Restablecer demo</button><div className="support-card"><b>Venta consultiva B2B</b><small>Diagnóstico → solución → aprobación</small></div></div>
    </aside>
    {open && <button className="backdrop" onClick={() => setOpen(false)} aria-label="Cerrar menú"/>}
    <div className="workspace">
      <header className="app-header"><button className="icon-button mobile-only" onClick={() => setOpen(true)} aria-label="Abrir menú"><Menu size={22}/></button><div className="header-title">INDEX CONDO <span>•</span> Comercial</div><div className="header-actions"><Link href="/prospectos?nuevo=1" className="button button-primary compact"><Plus size={17}/> Nuevo prospecto</Link><label className="user-switcher"><span className="avatar">{currentUser.fullName.split(" ").map((n) => n[0]).slice(0,2).join("")}</span><span className="user-copy"><b>{currentUser.fullName}</b><small>{roleLabels[currentUser.role]}</small></span><ChevronDown size={15}/><select value={currentUser.id} onChange={(e) => setCurrentUser(users.find((u) => u.id === e.target.value) ?? users[0])} aria-label="Cambiar usuario de demostración">{users.filter((u) => u.active).map((u) => <option value={u.id} key={u.id}>{u.fullName}</option>)}</select></label></div></header>
